...
class ww_DigTwin_basic_props(bpy.types.PropertyGroup):
    ...
    def update_length(self,context):
        in_loc = bpy.context.collection.children[context.active_node.name].objects['Rail Actuator_In'].location
        out_loc = bpy.context.collection.children[context.active_node.name].objects['Rail Actuator_Out'].location
        normalized = (out_loc-in_loc).normalized()
        bpy.context.collection.children[context.active_node.name].objects['Rail Actuator_Out'].location =\
